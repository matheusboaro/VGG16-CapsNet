...
def CapsNet(input_shape, n_class, routings):
    ...
    #conv1 = layers.Conv2D(filters=512, kernel_size=9, strides=1, padding='valid', activation='relu', name='conv1')(x)
    ...
    #primarycaps = PrimaryCap(conv1, dim_capsule=8, n_channels=32, kernel_size=3, strides=2, padding='valid')
...
def test(model, data, args):
    ...
    confusion=confusion_matrix(np.argmax(y_test, 1), np.argmax(y_pred, 1))
    report= classification_report(np.argmax(y_test, 1), np.argmax(y_pred, 1))
    print(report)
    print (confusion)
    dice,jaccard,sensitivity,specificity,accuracy,auc=get_metric(np.argmax(y_test, 1), np.argmax(y_pred, 1))
...
def load_mnist():
    filex='/home/boaro/Área de Trabalho/CapsNet 0.4/trainX.npy'
    #file_streamx = file_io.FileIO(filex, mode='r')
    x_train = np.load(filex)
    filey='/home/boaro/Área de Trabalho/CapsNet 0.4/trainY.npy'
    #file_streamy = file_io.FileIO(filey, mode='r')
    y_train = np.load(filey) 

    filex='/home/boaro/Área de Trabalho/CapsNet 0.4/testX.npy'
    #file_streamx = file_io.FileIO(filex, mode='r')
    x_test = np.load(filex)
    filey='/home/boaro/Área de Trabalho/CapsNet 0.4/testY.npy'
    #file_streamy = file_io.FileIO(filey, mode='r')
    y_test = np.load(filey) 

    filex='/home/boaro/Área de Trabalho/CapsNet 0.4/validX.npy'
    x_val = np.load(filex)
    filey='/home/boaro/Área de Trabalho/CapsNet 0.4/validY.npy'
    y_val = np.load(filey)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lr_decay', default=0.369, type=float,
                        help="The value multiplied by lr at each epoch. Set a larger value for larger epochs")
    parser.add_argument('--lam_recon', default=0.25, type=float,
